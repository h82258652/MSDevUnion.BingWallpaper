...
namespace BingoWallpaper.ThirdParty.Helper
{
    /// <summary>
    /// 提供对可视化树的各种方法的简单封装
    /// </summary>
    public static class VisualThreeHelper
    {
        /// <summary>
        /// 在父控件中查找指定类型的子控件
        /// </summary>
        ...
        /// <returns>找到的控件</returns>
        public static DependencyObject FindVisualElement(DependencyObject container, Func<DependencyObject, bool> comparer)
        {
            var childQueue = new Queue<DependencyObject>();
        ...
        /// <param name="comparer">比较器</param>
        /// <returns>找到的控件</returns>
        public static List<DependencyObject> FindVisualElements(DependencyObject container, Func<DependencyObject, bool> comparer)
        {
            var childQueue = new Queue<DependencyObject>();
        ...
        public static FrameworkElement FindVisualElementFormName(FrameworkElement container, String name)
        {
            ...
            while (childQueue.Count > 0)
            {
                ...
                for (int childIndex = 0; childIndex < childCount; childIndex++)
                {
                    childQueue.Enqueue(Windows.UI.Xaml.Media.VisualTreeHelper.GetChild(current, childIndex) as FrameworkElement);
                }
            }
